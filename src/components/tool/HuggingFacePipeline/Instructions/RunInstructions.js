...
export function RunInstructions({service, tool, task, model, port}) {
    ...
    return <>
        ...
        {tool === "CLI" && <>
            ...
            <CopyBox>
                gradio pipeline {task} --model={model} --server-name="0.0.0.0" --server-port={port} --device-map="auto"
            </CopyBox>
        </>}
        {tool === "Script" && <>
            <InstructionText>Save the following to a file named <Code>hf_pipeline.py</Code></InstructionText>
            <CopyBox>
                {`import gradio
import transformers
import sys
...
`}
            ...
            <CopyBox>
                python hf_pipeline.py {port}
            </CopyBox>
        </>}
        {tool === "Notebook" && <>
            {isWorkstation && <>
                <InstructionText>Run the following command to start the server:</InstructionText>
                <CopyBox>
                    python -m jupyterlab
                </CopyBox>
                <InstructionText>Open the link to the jupyter server in your browser, and create a new
                    notebook.</InstructionText>
            </>}
            <InstructionText>Execute the following code in the notebook:</InstructionText>
            <CopyBox>
                {`import gradio
import transformers
