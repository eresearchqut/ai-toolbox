        ...
        >
            <InputGroup>
                ...
                />
                {
                    model &&
                    <InputRightElement>
                        <Button size="xs" onClick={() => onModelChange("")}><CloseIcon/></Button>
                    </InputRightElement>
                }
...
const formatModelParameters = (modelParameters) =>
    Object.entries(modelParameters).map(([precision, value]) => {
        const [name, bits] = MODEL_FORMATS[precision];
        return `${formatNumber(value)} ${name} (${bits} bits)`;
    }).join(", ");
const formatModelMinSize = (modelParameters) =>
    `${formatNumber(Object.entries(modelParameters).reduce((acc, [precision, value]) => {
            ...
            <TableContainer>
                <Table variant="simple" size="sm">
                    <Tbody>
                        ...
                        <Tr>
                            <Th>Downloads</Th>
                            <Td>{formatNumber(modelData?.downloads ?? knownModelData?.downloads)}</Td>
                        </Tr>
                        {
                        ...
                        }
                        {
                            ...
                            <>
                                <Tr>
                                    <Th>Parameters</Th>
                                    <Td>{formatModelParameters(modelData.safetensors.parameters)}</Td>
                                </Tr>
                                <Tr>
                                    <Th>
                                        <Tooltip
                                            label="Minimum size of the model in memory according to an approximate calculation. Does not include memory required for inference.">
                                            <span>Minimum Size</span>
                                        </Tooltip>
                                    </Th>
                                    <Td>{formatModelMinSize(modelData.safetensors.parameters)}</Td>
                                </Tr>
                            </>
                        }
                        {
                            (modelData?.tags ?? knownModelData?.tags) &&
                            <Tr>
                                <Th>Tags</Th>
                                <Td>
                                    <Flex gap={2} flexWrap={"wrap"}>
                                        {
                                            (modelData?.tags ?? knownModelData?.tags).map(tag =>
                                                <Tag size="md" key={tag}>
                                                    <Link
                                ...
                                <Td>
                                    <Flex gap={2} flexWrap={"wrap"}>
                                        {modelData.config.architectures.map(architecture =>
                                            <Tag size="md" key={architecture}>{architecture}</Tag>)
                                        }
                                    </Flex>
                                </Td>
                            </Tr>
                        ...
                        }
                        {
                            ...
                            <Tr>
                                <Th>CO2 Emissions</Th>
                                <Td>{formatNumber(modelData.cardData.co2_eq_emissions.emissions / 1000)} kg</Td>
                            </Tr>
                        }
                    </Tbody>
                </Table>
...
export function Model({model, onModelChange, task}) {
    const {data: modelsData, error: modelsError, loading: modelsLoading} = useData({
    ...
    if (modelsError) {
        return <Alert status='error'>Error retrieving models: {modelsError.message}</Alert>;
